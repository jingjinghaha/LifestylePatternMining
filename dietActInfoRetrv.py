...
def getDaysList(subjectID):
    ...
    for row in range(8,sheet.nrows):
        if sheet.cell_value(row,0):
            ...
            year = np.str(data).split('.')[2]
            month = np.str(data).split('.')[0]
            day = np.str(data).split('.')[1]
            date_to_get = date(int(year),int(month),int(day))
            days.append(calendar.day_name[date_to_get.weekday()][0:3]) 
    return days
