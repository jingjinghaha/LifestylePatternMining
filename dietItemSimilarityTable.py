...
def dietItemSimilarityDict(dist = 'jaccard'):
	dietSimilarity_dict = {} 
	available_list = ['039','044','045','048','049','050','051','052','053','054','056','057','058','059','060','061','063','064','065','066','067','068','069','070','071','072','073','074','075']
	# i = 0
	if dist == 'TFIDF':
		dietSimilarity_dict = utilise.TFIDF('dietItem')
	else: 
...
def excelTable(dist = 'jaccard'):
	...
	dietSimilarity_dict = dietItemSimilarityDict(dist)
	for subjectID in available_list:
		row_index += 1
		col_index = 0
	...
	workbook.save('dietItemSimilarityTable_'+dist+'.xls')

def dietItemSimilarityTable(dist = 'jaccard'):
	print 'in dietItemSimilarityTable()'
	excelTable(dist)


# data visualization (table) for the diet Item similarity between two users 
# dietItemSimilarityTable()
